    ...
    ~ {
      ...
      bd <- map(
        ...
        ~ {
          bd %>%
            mutate(
              "dif_{stringr::str_remove(eleccion_referencia, 'ele_')}_{stringr::str_sub(string = .x, start = -5, end = -1)}" := !!sym(
                eleccion_referencia
              ) -
                !!sym(.x)
            )
  ...
  res <- map(
    ...
    ~ {
      ...
      bd %>%
        ...
        summarise(across(
          any_of(sufijo),
          ~ sum(.x, na.rm = T) / sum(!!sym(nominal), na.rm = T)
        )) %>%
        filter(!is.na({{ grupo }}))
    }
  ) %>%
...
calcular_votos_totales <- function(bd, partido, eleccion, grupo = NULL) {
  res <- bd %>%
    {
      if (!rlang::quo_is_null(enquo(grupo))) bd %>% group_by({{ grupo }}) else .
    } %>%
    summarise(across(
      matches(
        cross(list(partido, eleccion)) %>%
...
ganador_eleccion <- function(bd, eleccion, tipo = NULL, nivel, partido = NULL) {
  ...
  res <- bd %>%
    ...
    mutate(
      ganador = pmap(
        across(
          ends_with(glue::glue("_{eleccion}")) &
            -contains("_nominal_") &
            -contains("_total_")
        ),
        ~ names(c(...)[which.max(c(...))])
...
crear_mapa_electoral <- function(bd, eleccion, año, grupo = NA, partido = NA) {
  ...
  g <- pca_modelo %>%
    ...
    labs(
      ...
      subtitle = glue::glue(
        "VE {scales::percent(last(calidad$cumulative),accuracy=1)}"
      ),
      x = "CP1 ()",
      y = "CP2 ()",
      caption = glue::glue(
        "{scales::comma(nrow(base), accuracy=1)} observaciones"
      )
    ) +
    theme_void()
  # Output
...
graficar_sankey_ganadores <- function(bd, elecciones, unidad_analisis) {
  ...
  bd <- bd %>%
    ...
    mutate(
      node = forcats::fct_lump(node, n = 6, other_level = "Otros"),
...
graficar_cloropeta <- function(bd, shp, colores_nombrados, eleccion, grupo) {
  ...
  bd <- degradar_color_partido(
    bd,
    nombre = partido,
    variable = votos,
    colores_nombrados = colores_nombrados,
    valor_maximo = valor_referencia
  )
  res <- bd %>%
    split(list(.$partido, .$eleccion, .$ano)) %>%
    map(
      ~ {
        bd <- left_join(shp, .x, by = "seccion") %>%
          filter(!is.na(votos))
        ...
        ggplot() +
          ...
          labs(
            title = glue::glue(
              "{toupper(unique(bd$eleccion))} 20{unique(bd$ano)}"
            )
          )
      }
    )
...
#' @export
graficar_totales_eleccion <- function(
  bd,
  colores_nombrados,
  eleccion,
  grupo = NULL
  ...
  grafica <- bd %>%
    ggplot(aes(x = eleccion, y = resultado, group = partido)) +
    geom_line(aes(color = partido), alpha = 0.9) +
    geom_point(aes(group = eleccion, color = partido)) +
...
probar_independencia_ganador <- function(bd, ganador, eleccion, ...) {
  bd <- bd %>%
    mutate(
      triunfo = as.factor(!!sym(glue::glue("ganador_{eleccion}")) == ganador)
    )
  dots <- enquos(...)
  res <- map_df(
    dots,
    ~ {
      prueba_formula <- rlang::new_formula(
        rlang::quo_get_expr(.x),
        quote(triunfo)
      )
      prueba_t <- infer::t_test(
        formula = prueba_formula,
        alternative = "two-sided",
        x = bd
      )
      prueba_wc <- broom::tidy(wilcox.test(prueba_formula, data = bd))
      resultado <- tibble(
        explicativa = rlang::expr_text(enquo(.x)),
        p_prueba_t = prueba_t$p_value,
        p_prueba_wc = prueba_wc$p.value
      ) %>%
        mutate(
          determinacion_prueba_t = if_else(
            p_prueba_t > .05,
            "Rechazo dependencia",
            "Rechazo independencia"
          ),
          determinacion_prueba_wc = if_else(
            p_prueba_wc > .05,
            "Rechazo dependencia",
            "Rechazo independencia"
          )
...
crear_indice <- function(bd, partido, nivel) {
  # --- 1. Selección y limpieza inicial de datos ---

  # Encontrar nombres de columnas relevantes para el partido
  nombres <- names(bd)[grepl(glue::glue("pct_{partido}"), names(bd))]

  # Si no se encuentra ninguna columna, detenerse
  if (length(nombres) == 0) {
    warning(glue::glue(
      "No se encontraron columnas para el partido '{partido}'."
    ))
    return(NULL)
  }
  ...
  bd_partido <- bd |>
    dplyr::as_tibble() |>
    dplyr::select(dplyr::all_of(nivel), dplyr::all_of(nombres)) |>
    # ¡BLINDAJE! Convertir Inf y NaN a NA para poder manejarlos todos juntos.
    dplyr::mutate(dplyr::across(where(is.numeric), ~ dplyr::na_if(., Inf))) |>
    dplyr::mutate(dplyr::across(where(is.numeric), ~ dplyr::na_if(., -Inf))) |>
    dplyr::mutate(dplyr::across(where(is.numeric), ~ dplyr::na_if(., NaN))) |>
    # Se mantiene tu lógica original de reemplazar NA con 0.
    dplyr::mutate(dplyr::across(where(is.numeric), ~ tidyr::replace_na(.x, 0)))

  # --- 2. Filtro de columnas con varianza cero (método robusto) ---

  columnas_numericas <- bd_partido |> dplyr::select(where(is.numeric))

  # Se calculan las desviaciones estándar. Columnas constantes tendrán sd = 0.
  sds <- sapply(columnas_numericas, sd, na.rm = TRUE)

  # Nombres de las columnas que SÍ tienen varianza (son útiles para PCA)
  utiles <- names(sds[sds > 0])

  # --- 3. Salvaguarda: Verificar si hay suficientes variables para PCA ---

  # PCA necesita al menos 2 variables para funcionar.
  if (length(utiles) < 2) {
    warning(glue::glue(
      "No hay suficientes variables con varianza para calcular el índice de '{partido}' después de la limpieza."
    ))
    return(NULL)
  }
  ...
  # El select ahora es seguro, solo usa columnas válidas.
  pca_modelo <- bd_partido |>
    dplyr::select(dplyr::all_of(utiles)) |>
    stats::prcomp(scale. = TRUE)

  # --- 5. Estandarización y orientación del índice ---

  # Extraer la orientación de los pesos (loadings) del primer componente
  rotacion_pc1 <- pca_modelo$rotation[, "PC1"]

  # Generar las predicciones (scores) del PC1
  pred <- stats::predict(pca_modelo, newdata = bd_partido)[, "PC1"]

  # Si la suma de los pesos es negativa, se invierte el signo del índice.
  # Esto asegura que el índice siempre apunte en la misma "dirección".
  if (sum(rotacion_pc1) < 0) {
    pred <- pred * -1
  }

  ind <- (pred - mean(pred, na.rm = TRUE)) / sd(pred, na.rm = TRUE)

  # --- 6. Preparar y devolver el resultado final ---

  resultado_final <- bd |>
    dplyr::select(all_of(nivel)) |>
    dplyr::mutate(ind = ind) |>
    dplyr::rename_with(~ glue::glue("{partido}"), ind)

  return(dplyr::as_tibble(resultado_final))
}
...
colorear_indice <- function(bd, c_principal, var) {
  ...
  colorear <- leaflet::colorQuantile(
    grDevices::colorRamp(
      c(no_principal, "white", c_principal),
      space = "Lab",
      bias = 1.5,
      interpolate = "spline"
    ),
...
crear_quantiles <- function(bd, partido, grupos = 4) {
  bd |>
    mutate(
      quant = dvmisc::create_qgroups(!!rlang::sym(partido), groups = grupos),
...
crear_label <- function(bd, nivel) {
  votos <- bd |>
    ...
    summarise(
      label_v = paste(label, collapse = "<br>"),
      .by = c(!!rlang::sym(nivel), eleccion, ano)
    )
  ...
  if (sum(grepl("quant", names(bd))) > 0) {
    indice <- bd |>
      ...
      summarise(
        label_ind = paste(label, collapse = "<br>"),
        .by = c(!!rlang::sym(nivel))
      ) |>
      mutate(
        label_ind_2 = paste(
          "Seccion:",
          stringr::str_sub(seccion, -4, -1),
          "<br>",
          label_ind
        )
      )
  ...
  label <- votos %>%
    {
      if (exists("ganador")) {
        left_join(., ganador, by = join_by(!!sym(nivel), eleccion, ano))
      } else {
        .
      }
    } %>%
    {
      if (exists("indice")) {
        left_join(., indice, by = join_by(!!sym(nivel)))
      } else {
        .
      }
    } %>%
    transmute(
      ...
      label = paste(
        glue::glue("Seccion: {stringr::str_sub(seccion, -4, -1)}"),
        label_g,
        label_v,
        label_ind,
        sep = "<br>"
    ...
    rename_with(~ paste0("label_", .x), -!!rlang::sym(nivel)) %>%
    {
      if (exists("indice")) {
        left_join(
          .,
          select(indice, all_of(nivel), label_ind_2),
          by = join_by(!!sym(nivel))
        )
      } else {
        .
      }
