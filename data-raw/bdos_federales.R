...
bd_pr_18 <- pr_18 <- read_excel(
  "~/Dropbox (Selva)/Ciencia de datos/Consultoría Estadística/Recursos/Externos/Limpieza/Resultados definitivos/Federal/2018/Presidente_casilla.xlsx"
) %>%
  janitor::clean_names() %>%
  as_tibble()
...
setwd(
  "~/Dropbox (Selva)/Ciencia de datos/Consultoría Estadística/Recursos/Externos/Limpieza/Resultados definitivos/Federal/2018/diputados_federales"
)
...
bd_df_21 <- read_delim(
  "~/Dropbox (Selva)/Ciencia de datos/Consultoría Estadística/Recursos/Externos/INE/Bases de datos/2021/Computos Distritales/diputaciones.csv",
  delim = "|",
  escape_double = FALSE,
  trim_ws = TRUE,
...
bd_df_15 <- read_csv(
  "~/Dropbox (Selva)/Ciencia de datos/Consultoría Estadística/Recursos/Externos/Limpieza/Resultados definitivos/Federal/2015/Diputado_casilla.csv"
) %>%
  janitor::clean_names() %>%
  as_tibble()

bd_df_12 <- read_csv(
  "~/Dropbox (Selva)/Ciencia de datos/Consultoría Estadística/Recursos/Externos/Limpieza/Resultados definitivos/Federal/2012/Diputado_casilla.csv"
) %>%
  janitor::clean_names() %>%
  as_tibble()
...
sen_12_e <- sen_12 |>
  ...
  mutate(
    clave_casilla = glue::glue(
      "{estado}{seccion}{tipo_casilla}{sprintf('%02s', id_casilla)}{sprintf('%02s', ext_contigua)}"
    )
  )
...
df21 <- df21 %>%
  rename_with.(~ paste0('ele_', .x), .cols = pan:nominal)
...
pr18 <- bd_pr_18 %>%
  mutate(
    municipio = gsub(pattern = "( |)[0-9]", replacement = "", x = municipio)
  )
...
final_pr18 <- final_pr18 %>%
  mutate(
    id_casilla = case_when(
      nchar(casilla) == 1 ~ paste0(casilla, "0100"),
...
df15 <- df15 %>%
  rename("noreg" = no_reg, "distritof" = distrito, "pes" = ps) %>%
  mutate(
    across(pan:nominal, ~ as.numeric(.x)),
    estado = formatC(estado, width = 2, flag = "0"),
    seccion = formatC(seccion, width = 4, flag = "0"),
...
df12 <- df12 %>%
  ...
  rename("noreg" = no_reg, distritof = distrito, nominal = lista_nominal) %>%
  mutate(
    across(pan:nominal, ~ as.numeric(.x)),
    estado = formatC(estado, width = 2, flag = "0"),
    seccion = formatC(seccion, width = 4, flag = "0"),
...
df_18 %>%
  ...
  saveRDS("inst/electoral/nacional/df_18.rda")
...
aux <- read_delim(
  ...
  rename(
    ...
    !!glue::glue(
      "distritof_{readr::parse_number(eleccion)}"
    ) := id_distrito_federal,
    !!glue::glue(
      "nombre_distritof_{readr::parse_number(eleccion)}"
    ) := distrito_federal
  ) |>
  rename_with(
    ~ paste("ele", .x, eleccion, sep = "_"),
    .cols = c(pan:nominal)
  ) |>
  mutate(
    estado = sprintf("%02s", id_entidad),
...
aux <- readr::read_delim(
  paste0(path, "/pr_24/PRES_2024.csv"),
  delim = "|",
  skip = 7,
...
readr::write_rds(
  aux,
  file = glue::glue("inst/electoral/{estado}/{eleccion}.rda")
)
...
aux <- readr::read_delim(
  paste0(path, "/df_24/DIP_FED_2024.csv"),
  delim = "|",
  skip = 7,
  locale = readr::locale(encoding = "ISO-8859-1")
) |>
...
aux <- read_delim(
  paste0(path, "/sen_24/SEN_2024.csv"),
  delim = "|",
  skip = 7,
  locale = locale(encoding = "ISO-8859-1")
) |>
