  ...
  bd |>
    mutate(
      ...
      clave_casilla = glue::glue(
        "{estado}{str_pad(seccion,pad = '0', width = 4)}{tipo_casilla}{str_pad(id_casilla,pad = '0', width = 2)}{str_pad(ext_contigua,pad = '0', width = 2)}"
      )
    ) |>
...
homologar_bd_pue <- function(bd) {
  bd |>
    mutate(
      casilla = if_else(
        grepl("EXTRAORDINARIA", casilla) & grepl("CONTIGUA", casilla),
        gsub("CONTIGUA ", "", casilla),
        casilla
      ),
    ...
    tidyr::separate(
      casilla,
      into = c("tipo_casilla", "id_casilla", "ext_contigua"),
      remove = FALSE
    ) |>
    tidyr::replace_na(replace = list(ext_contigua = "00", id_casilla = "01")) |>
    mutate(
      clave_casilla = glue::glue(
        "21{sprintf('%04s', seccion)}{tipo_casilla}{sprintf('%02s', id_casilla)}{sprintf('%02s',ext_contigua)}"
      )
    )
}
# Relación 18 --------------------------------------------------------
df_18 <- read_rds("inst/electoral/nacional/df_18.rda") |>
  ...
  distinct(
    clave_casilla,
    distritof_18 = distritof,
    nombre_distritof_18 = nombre_distritof,
    municipio_18 = municipio,
    seccion,
    nominal = ele_nominal_df_18
  )
...
pm_21 <- read_csv(path, skip = 2) |>
  ...
  distinct(
    ...
    seccion,
    casilla = paste0(tipo_casilla, id_casilla)
  ) |>
...
aux <- aux |>
  ...
  rename_with(~ paste0('ele_', .x, "_", elec), .cols = pan:nominal)
...
nombres_mun <- read_rds("inst/electoral/pue/pm_18.rda") |>
  distinct(
    nombre_municipio_21 = nombre_municipio_18,
    municipio_21 = municipio_18
  )
...
aux <- readxl::read_excel(path, skip = 4) |>
  ...
  select(
    ...
    estado,
    clave_casilla,
    distritol_21:id_tipo_candidatura,
    pan:cand_ind_2,
    nominal:total,
...
final <- aux |>
  mutate(
    across(pan:total, ~ as.numeric(.x)),
    seccion = formatC(seccion, width = 4, flag = "0"),
    distritol_21 = formatC(distritol_21, width = 2, flag = "0")
  ) |>
  rename_with(~ paste0('ele_', .x, "_", elec), .cols = pan:total)
