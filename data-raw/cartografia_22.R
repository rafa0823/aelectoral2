...
wd %>%
  ...
  map(
    ~ {
      ...
      sf <- st_read(.x)
      if ("DISTRITO" %in% names(sf)) {
        sf <- sf |> rename(DISTRITO_F = DISTRITO)
      }
      temp <- sf %>%
        transmute(
          distritof_22 = paste(
            formatC(ENTIDAD, width = 2, flag = 0),
            formatC(DISTRITO_F, width = 2, flag = 0),
            sep = "_"
          )
        )
    ...
    ~ {
      ...
      temp <- st_read(.x) %>%
        transmute(
          distritol_22 = paste(
            formatC(ENTIDAD, width = 2, flag = 0),
            formatC(DISTRITO_L, width = 2, flag = 0),
            sep = "_"
          )
        )
    ...
    ~ {
      ...
      temp <- st_read(.x) %>%
        transmute(
          municipio_22 = paste(
            formatC(ENTIDAD, width = 2, flag = 0),
            formatC(MUNICIPIO, width = 3, flag = 0),
            sep = "_"
          ),
  ...
  map(
    ~ {
      list.files(.x, pattern = "ENTIDAD.shp", full.names = T)[
        !grepl(
          pattern = ".xml",
          x = list.files(.x, pattern = "ENTIDAD.shp", full.names = T),
          fixed = T
        )
      ]
    }
    ...
    ~ {
      ...
      temp <- st_read(.x) %>%
        transmute(
          ...
          distritof_22 = paste(
            formatC(ENTIDAD, width = 2, flag = 0),
            formatC(DISTRITO, width = 2, flag = 0),
            sep = "_"
          ),
          distritol_22 = paste(
            formatC(ENTIDAD, width = 2, flag = 0),
